...
describe('Wallet Operations Integration Tests', () => {
  test('should display total balance split between promo and non-promo', async () => {
  ...
  test('should credit promotional amount to promo balance', async () => {
    const { user, wallet } = await createUserWithWallet({
      promoBalance: 0
    });
  ...
  test('should deduct from wallet for booking payment', async () => {
    const { user, wallet } = await createUserWithWallet({
      nonPromoBalance: 200
    });
  ...
  test('should use promo balance first, then non-promo', async () => {
    const { user, wallet } = await createUserWithWallet({
      promoBalance: 50,
      nonPromoBalance: 100
    });
